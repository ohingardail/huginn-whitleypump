Agent.receive = function() {
	...
	var search_objects = {
		...
		park		: this.credential('data-park-town-constituency-ward-postcode'),
		busroute	: this.credential('data-busroute-ward'),
		stop		: this.credential('data-stop-search')
	}

	var events = this.incomingEvents();
  
	for (var i = 0; i < events.length; i++) {
	  //this.log("event : " + JSON.stringify(events[i].payload) )
			...
			for (var s = 0; s < search_table.length; s++)
			{
				...
				switch (search_type) 
				{
					...
					case 'road':
						...
						search_term_regex=search_term_regex.replace(/\bstreet\b/ig, 	'st(?:re+t)?')
						search_term_regex=search_term_regex.replace(/\bgardens\b/ig, 	'g(?:ar)?de?ns?')
						search_term_regex=search_term_regex.replace(/\bclose\b/ig, 	'clo?s?e?')
						search_term_regex=search_term_regex.replace(/\blane\b/ig, 	'la?n?e?')
						...
						search_term_regex=search_term_regex.replace(/\bview\b/ig, 	'v(?:ie)?w')
						search_term_regex=search_term_regex.replace(/\bwalk\b/ig, 	'wa?l?k')
						break;
					case 'ward':
					  search_term_regex=search_term_regex.replace(/\bchurch\b/ig, 	'church ward')
						break;
					case 'person':
						break;
					case 'park':
				...
				var search_term_regex = '\\b' + search_term_regex.trim().replace(/\s+/g, '\\s+').replace('-', '[ -]?') + '\\b'
        // to ignore repeated chars: .replace(/(.)\1+/ig, '$1+') - map = mapp, kennett = kenet
        // to make 'the' optional : .replace(/\bthe\\s\+\b/ig, '(?:the\\s+)?')
		...
		// append to event if no stop code found
		if (Object.keys(found).length > 0 && !( Object.keys(found).includes("stop") ) )
		{
